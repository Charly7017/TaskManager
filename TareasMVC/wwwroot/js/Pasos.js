...
function manejarClickCheckboxPaso(paso) {
    ...
    console.log(pasosRealizadosActual)
...
async function actualizarOrdenPasos() {
    const ids = await obtenerIdsPasos()
    console.log(ids)
    await enviarIdsPasosAlBackend(ids)

    const arregloOrdenado = tareaEditarVM.pasos.sorted(function (a,b) {
        return ids.indexOf(a.id().toString()) - ids.indexOf(b.id().toString()); 
    })

    //tareaEditarVM.pasos([])
   
    tareaEditarVM.pasos(arregloOrdenado)

}

async function obtenerIdsPasos() {
