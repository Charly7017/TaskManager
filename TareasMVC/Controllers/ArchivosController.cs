...
namespace TareasMVC.Controllers
{
    ...
    public class ArchivosController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<ArchivoAdjunto>>> Post(int tareaId, [FromForm] IEnumerable<IFormFile> archivos)
        {
            ...
            var tarea = await context.Tareas.FirstOrDefaultAsync(p=>p.Id==tareaId);
            ...
            var existenArchivosAdjuntos =
                    await context.ArchivosAdjuntos.AnyAsync(p => p.TareaId == tareaId);
        ...
        public async Task<IActionResult> Put(Guid id, [FromBody] string titulo)
        {
            ...
            var archivoAdjunto = await context.ArchivosAdjuntos.Include(p => p.Tarea).FirstOrDefaultAsync(p => p.Id == id);
        ...
        public async Task<IActionResult> Ordenar(int tareaId, [FromBody] Guid[] ids)
        {
            ...
            var tarea = await context.Tareas.FirstOrDefaultAsync(p => p.Id == tareaId && p.UsuarioCreacionId == usuarioId);
            ...
            var archivosAdjuntos = await context.ArchivosAdjuntos.Where(p => p.TareaId==tareaId).ToListAsync();

            var archivosIds = archivosAdjuntos.Select(p => p.Id);

            var idsArchivosNoPertenecenALaTarea = ids.Except(archivosIds).ToList();

            if (idsArchivosNoPertenecenALaTarea.Any())
            {
                return BadRequest("No todos los archivos no estan presentes");
            }
